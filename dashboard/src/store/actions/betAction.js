...
export const betAction  = (bet, history) => async dispatch =>{
    try {
        const result  = await Axios.post(`https://server.hosttesting.xyz/bet/post-title`, bet)
        if (result.data._id) {
...
export const resultAction = (question) => async dispatch => {
    try {
        await Axios.post(`https://server.hosttesting.xyz/bet/add-bet`, question)
        dispatch({
            type: Types.SET_ALERT,
            payload: {
...
export const loadAllBet = (gameId) => async dispatch => {
    try {
        const data = await Axios.get(`https://server.hosttesting.xyz/bet/get-all-bet?gameId=${gameId}`);
        dispatch({
            type: Types.SET_BET,
            payload: {
                bet: data.data.data
...
export const betActionAction = (bet, index, bets) => async dispatch => {
    try {
        const response = await Axios.put(`https://server.hosttesting.xyz/bet/bet-update`, bet);
...
export const resultShowAction = (result, resultIndex, bet, index, bets) => async dispatch => {
    try {
        const res = await Axios.put(`https://server.hosttesting.xyz/bet/result-update`,result )
