...
export const betAction  = (bet, history) => async dispatch =>{
    try {
        const result  = await Axios.post(`http://localhost:4000/bet/post-title`, bet)
        if (result.data._id) {
...
export const resultAction = (question) => async dispatch => {
    try {
        await Axios.post(`http://localhost:4000/bet/add-bet`, question)
        dispatch({
            type: Types.SET_ALERT,
            payload: {
...
export const loadAllBet = (gameId) => async dispatch => {
    try {
        const data = await Axios.get(`http://localhost:4000/bet/get-all-bet?gameId=${gameId}`);
        dispatch({
            type: Types.SET_BET,
            payload: {
                bet: data.data.data
...
export const betActionAction = (bet, index, bets) => async dispatch => {
    try {
        const response = await Axios.put(`http://localhost:4000/bet/bet-update`, bet);
...
export const resultShowAction = (result, resultIndex, bet, index, bets) => async dispatch => {
    try {
        const res = await Axios.put(`http://localhost:4000/bet/result-update`,result )
