...
export const gameAddAction = (game, history, data) => async dispatch =>{
    try {
        const result = await Axios.post('https://b24win.herokuapp.com/game/add', game);
...
export const gameLodeAction = page => async dispatch =>{
     try {
        const loadGame = await Axios.get(`https://b24win.herokuapp.com/game/get-all?page=${page}`)
...
export const gameStatusAction = (game , index, gameArr, length) => async dispatch => {
    try {
        ...
        const newGame = await Axios.patch('https://b24win.herokuapp.com/game/game-update', game)
