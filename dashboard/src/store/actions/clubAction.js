...
export const clubAction = club => async dispatch =>{
    try {
        ...
        const res = await axios.post(`https://b24win.herokuapp.com/club/add-club`, club)
          dispatch({
...
export const loadAllClub = (page) => async dispatch => {
    try {
        const res = await axios.get(`https://b24win.herokuapp.com/club/get-all-club?page=${page}`);
        dispatch({
            type: Types.SET_CLUB,
            payload:{
                club: res.data.club,
