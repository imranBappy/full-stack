...
export const userBetGetAction = (page) => async dispatch => {
    try {
        const bet = await axios.get(`https://b24win.herokuapp.com/usersbet?page=${page}`);
        dispatch({
...
export const betStatusAction = (result, resultIndex, allResult, c, status, singleBet, betIndex, allBet) => async dispatch => {
    try {
        ...
        const res = await axios.patch(`https://b24win.herokuapp.com/usersbet/result-status-update?resultId=${result._id}&status=${result.status}`)
