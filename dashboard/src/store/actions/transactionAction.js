...
export const allTransactionGetAction = (page, transaction) => async dispatch =>{
    try {
        const res = await axios.get(`http://localhost:4000/transaction?transaction=${transaction}&page=${page}`);
...
export const transactionAcceptAction = (transaction,index, rows, length, status, path) => async dispatch =>{
    try {
        ...
        const updateTransaction = await axios.patch(`http://localhost:4000/transaction/update-transaction/${transaction._id}?status=${transaction.status}&userId=${transaction.user._id}`);
        dispatch({
            type: Types.SET_ALERT,
            payload:{
