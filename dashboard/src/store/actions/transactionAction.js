...
export const transactionAcceptAction = (transaction,index, rows, length, status, path) => async dispatch =>{
    try {
        ...
        const updateTransaction = await axios.patch(`https://b24win.herokuapp.com/transaction/update-transaction/${transaction._id}?status=${transaction.status}&userId=${transaction.user._id}`);
        dispatch({
            type: Types.SET_ALERT,
            payload:{
