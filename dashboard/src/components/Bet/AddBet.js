...
const AddBet = (props) => {
    ...
    useEffect(() =>{
        if (query.get("resultId")) {
            axios.get(`https://b24win.herokuapp.com/bet/get-single-bet?betId=${query.get("resultId")}`).then(res =>{
    ...
    const handleUpdate = () =>{
        if (checkValid()) {
            setIsValid(true);
            axios.put(`https://b24win.herokuapp.com/bet/result-update`, game).then(res=>{
