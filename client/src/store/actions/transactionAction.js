...
export const TransactionInputRequestAction = (TransactionInput, user) => async dispatch => {
    try {
        const res = await axios.post('http://localhost:4000/transaction/add', {...TransactionInput, user: user._id});
...
export const transitionGetAction = (page, user) => async dispatch => {
    try {
        const res = await axios.get(`http://localhost:4000/transaction?page=${page}&user=${user}`);
